...
def register():
    if request.method == "POST":
        ...
        uniqueID = username[:4] + usermobile[6:]

        cur.execute("INSERT INTO login_credentials(username, email, mobile, password, unique_id) VALUES (%s, %s, %s, %s, %s)", (username, useremail, usermobile, password, uniqueID))
...
def main():
    if account_true == True:
        ...
        cur = mysql.connection.cursor()
        cur.execute("SELECT unique_id FROM login_credentials WHERE username = %s;", [username])
        uniqueID = cur.fetchone()[0]

        mysql.connection.commit()
        cur.close()

        return render_template('session.html', username=username, uniqueID=uniqueID)
